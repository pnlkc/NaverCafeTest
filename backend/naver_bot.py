...
class NaverCafeBot:
    ...
    async def is_article_already_in_cafe(self, title: str) -> bool:
        """
        네이버 카페 정식 뉴스 게시판(/menus/{board_id})의 실시간 게시글 목록을 긁어와
        정밀 유사도 알고리즘(75%+ 일치)으로 카페 내 중복 게시글 존재 여부를 100% 실측 검증합니다.
        """
        self.settings = load_settings()
        club_id = self.settings.cafe.club_id
        board_id = getattr(self.settings.news, "publish_board_id", "4") or "4"
        ...
        # 네이버 카페 정식 뉴스 게시판 목록 URL
        menu_url = f"https://m.cafe.naver.com/ca-fe/web/cafes/{club_id}/menus/{board_id}"
        import difflib
        ...
        try:
            async with async_playwright() as p:
                ...
                found_titles = await page.evaluate("""() => {
                    const titles = [];
                    const selectors = ['a.mainLink', 'a[href*="articles/"]', '.article_title', '.txt_area', 'strong.tit', 'span.tit'];
